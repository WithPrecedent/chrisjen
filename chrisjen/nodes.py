...
class Parameters(amos.Dictionary):
    ...
    become apparent during execution of an chrisjen project, to create a unified 
    ...
    If an chrisjen class uses a Parameters instance, the 'finalize' method should
    be called before that instance's 'implement' method in order for each of the
    parameter types to be incorporated.
    
    Args:
        contents (Mapping[str, Any]): keyword parameters for use by an chrisjen
        ...
        name (str): designates the name of a class instance that is used for 
            internal referencing throughout chrisjen. To properly match parameters
...
@dataclasses.dataclass
class Component(amos.Node, workshop.LibraryFactory):
...
class Worker(Component, abc.ABC):
    """Keystone class for parts of an chrisjen workflow.
