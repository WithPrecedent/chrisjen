...
Classes:
    Graph (Bunch): a lightweight directed acyclic graph (DAG) that serves as
        the base class for chrisjen composite  Internally, the graph is
...
class System(containers.Lexicon, composites.Graph):
    ...
    def add(
        ...
        if ancestors is not None:  
            if utilities.is_property(item = ancestors, instance = self):
                ...
                ancestors = utilities.iterify(item = ancestors)
                missing = [n for n in ancestors if n not in self.contents]
                if missing:
                    raise KeyError(
                        f'ancestors {str(missing)} are not in {self.__name__}')
                else:
                    start = ancestors
    ...
    def merge(self, item: Union[composites.Composite]) -> None:
        ...
        if isinstance(item, System):
            ...
            adjacency = utilities.edges_to_adjacency(item = item)
        elif isinstance(item, composites.Matrix):
            adjacency = utilities.matrix_to_adjacency(item = item)
        elif isinstance(item, (list, tuple, set)):
            adjacency = utilities.pipeline_to_adjacency(item = item)
    ...
    def _find_all_paths(self, starts: composites.Nodes, stops: composites.Nodes) -> composites.Pipeline:
        ...
        Returns:
            composites.Pipeline: list of all paths through the System from all 'starts' 
                to all 'ends'.
    ...
    def __add__(self, other: Union[composites.Composite]) -> None:
        ...
        Args:
            other (Union[composites.Composite]): another Graph, an adjacency list, an 
                edge list, an adjacency matrix, or one or more nodes.
...
class Network(composites.Graph):
    ...
    def adjacency(self) -> composites.Adjacency:
        """Returns the stored graph as an adjacency list."""
        return utilities.matrix_to_adjacency(item = self.contents)
    ...
    def _stringify(self, node: Any) -> str:
        ...
        """        
        if isinstance(node, composites.Node):
            return node
        else:
            try:
                return node.name
                ...
                try:
                    return utilities.snakify(node.__name__)
                except AttributeError:
                    return utilities.snakify(node.__class__.__name__)
